...
public class ClassRemapper {
    ...
    public ClassRemapper(File directory, File jarBackupDirectory) throws FileNotFoundException, IOException, ClassNotFoundException {
        //TODO
        //this(new DefaultLogImpl(null), ClassLoader.getSystemClassLoader(), directory, jarBackupDirectory);
    ...
    private boolean rewriteJar(File file, int depth)
        ...
        try {
            ...
            if (manifest == null) {
                ...
                manifest.getMainAttributes().putValue("New-Relic-Backup-Hash", generateHash(file));
            ...
            while (entries.hasMoreElements()) {
                ...
                try {
                    ...
                    JarEntry newEntry = new JarEntry(entry.getName());
                    jos.putNextEntry(newEntry);
                    if ((!entry.getName().contains("com/newrelic/agent/")) && (entry.getName().endsWith(".class"))) {
                        ByteArrayOutputStream classBytes = new ByteArrayOutputStream();
                        Streams.copy(is, classBytes);
                        classBytes.close();
                        ClassData classData = visitClassBytes(classBytes.toByteArray());
                        ByteArrayInputStream byteStream = new ByteArrayInputStream(classData.getMainClassBytes());
                        try {
                        ...
                        if (classData.isShimPresent()) {
                            ...
                            JarEntry shimEntry = new JarEntry(classData.getShimClassName() + ".class");
                            jos.putNextEntry(shimEntry);
                            ByteArrayInputStream shimByteStream = new ByteArrayInputStream(classData.getShimClassBytes());
                            try {
    ...
    public boolean rewriteClass(File file)
        ...
        if ((classData.getMainClassBytes() != null) && (classData.isModified())) {
            this.log.info(MessageFormat.format("[{0}] modified classfile {1}", new Object[]{this.context.getClassName().replaceAll("/", "."), file.getName()}));
            Streams.copyBytesToFile(file, classData.getMainClassBytes());

            if (classData.isShimPresent()) {
                FileOutputStream out = new FileOutputStream(this.outputDirectory.getAbsolutePath() + File.separator + classData.getShimClassName() + ".class");
                try {
    ...
    private byte[] getBytes(File file)
            throws Exception {
        ByteArrayOutputStream originalBytes = new ByteArrayOutputStream((int) file.length());
    ...
    private ClassData visitClassBytes(byte[] bytes) {
        try {
            ...
            if (!this.context.hasTag("Lcom/newrelic/agent/android/instrumentation/Instrumented;")) {
                ClassVisitor cv = new AnnotatingClassVisitor(cw, this.context, this.log);
